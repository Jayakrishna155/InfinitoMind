document.addEventListener('DOMContentLoaded', function () {
  ...
  const messages = [
    { percentage: '500%', benefit: 'increased skills' },
    { percentage: '300%', benefit: 'better job opportunities' },
    { percentage: '400%', benefit: 'knowledge retention' },
];
const ecoGrid = document.querySelector(".eco-grid");

// Apply normal layout on mouse enter
ecoGrid.addEventListener("mouseenter", function () {
    ecoGrid.classList.add("normal");
    ecoGrid.classList.remove("fanned");
});

// Apply fanned layout on mouse leave
ecoGrid.addEventListener("mouseleave", function () {
    ecoGrid.classList.add("fanned");
    ecoGrid.classList.remove("normal");
});
...
function updateContent() {
    ...
    const message = messages[ci];

    // Update the paragraph content
    courseInfo.innerHTML = `<strong>${message.percentage}</strong> ${message.benefit} achieved by learners after completing the e-learning course*`;

    // Update the index for the next message
    ci = (ci + 1) % messages.length; // Loop back to the start
}

// Change content every 3 seconds (3000 milliseconds)
setInterval(updateContent, 3000);
  ...
  function renderStories() {
      ...
      for (let i = currentIndex; i < currentIndex + 3 && i < successStories.length; i++) {
